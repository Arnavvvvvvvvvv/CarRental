...
const ManageCars = () => {
  ...
  const toggleAvailability= async(carId)=>{
    try{
      const {data}= await axios.post('/api/owners/toggle-availability', {carId});
      if(data.success){
    ...
    <div className='px-4 pt-10 md:px-10 w-full'>
      ...
      <div className='max-w-3xl w-full rounded-md overflow-hidden border border-gray-600 mt-6'>
        <table className='w-full border-collapse text-left text-sm text-gray-600'>
          ...
          <tbody>
            {cars.map((car,index)=>(
              <tr key={index} className='border-t border-b-gray-600'>
                ...
                <td className='p-3'>{currency}{car.price_per_day}/day</td>
                ...
                <td className='flex items-center p-3'>
                  <img onClick={()=> toggleAvailability(car._id)} src={car.isAvailable? assets.eye_close_icon:assets.eye_icon} alt="edit" className='cursor-pointer'/>
